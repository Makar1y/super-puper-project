...
int template_to_html(FILE *html_template, char *target_filename) {
    ...
    *request_url = '.';
    ...
    while (fgets(buffer, LINE_BUFFER_SIZE, html_template)) {
        ++line;
        char *block_name_start = strstr(buffer, "{%"); // If start of block in line
        char *tag_name_start = strstr(buffer, "{{"); // if start of tag
        ...
        if (tag_name_start) { // tag serving
            char *tag_name_end = strstr(buffer, "}}");

            if (tag_name_end) {
                char tag_name[LINE_BUFFER_SIZE];
                if (sscanf(tag_name_start, "{{ %[^{}] }}", tag_name)) {
